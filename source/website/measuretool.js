...
export class MeasureTool {
  ...
  UpdatePanel() {
    ...
    if (this.settings.backgroundIsEnvMap) {
      ...
      let blendedColor = BlendBackgroundWithPageBackground(
    ...
    if (this.markers.length === 0) {
      this.panel.innerHTML = findName('SelectPoint');
    } else if (this.markers.length === 1) {
      this.panel.innerHTML = findName('SelectAnotherPoint');
    } else {
      ...
      if (calcResult.parallelFacesDistance !== null) {
        AddValue(
          ...
          findName('DistanceOfParallelFaces'),
