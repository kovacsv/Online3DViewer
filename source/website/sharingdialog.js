...
export function ShowSharingDialog(fileList, settings, viewer) {
  ...
  function AddCopyableTextInput(parentDiv, getText) {
    let copyText = findName('Copy');
    let copiedText = findName('Copied');
  ...
  if (!fileList.IsOnlyUrlSource()) {
    return ShowMessageDialog(
      findName('SharingFailed'),
      findName('textShareFail'),
      null
    );
  }
  ...
  let contentDiv = dialog.Init(findName('Share'), [
    {
      name: findName('Close'),
      onClick() {
        dialog.Close();
