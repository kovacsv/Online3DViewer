  ...
  Resize() {
    ...
    if (contentWidth < this.limits.minCanvasWidth) {
      ...
      if (neededIncrease > 0 && isNavigatorVisible) {
        let navigatorDecrease = Math.min(
          neededIncrease,
          leftWidth - this.limits.minPanelWidth
        );
      ...
      if (neededIncrease > 0 && isSidebarVisible) {
        let sidebarDecrease = Math.min(
          neededIncrease,
          rightWidth - this.limits.minPanelWidth
        );
    ...
    let introContentHeight = GetDomElementOuterHeight(
      this.parameters.introContentDiv
    );
    let introContentTop = (contentHeight - introContentHeight) / 3.0;
    this.parameters.introContentDiv.style.top =
      introContentTop.toString() + 'px';
...
export class Website {
  constructor(parameters) {
    ...
    this.layouter = new WebsiteLayouter(
      this.parameters,
      this.navigator,
      this.sidebar,
      this.viewer,
  ...
  OnModelContextMenu(globalMouseCoordinates, mouseCoordinates) {
    ...
    if (meshUserData === null) {
      items.push({
        name: findName('FitModelToWindow'),
        icon: 'fit',
        onClick: () => {
      ...
      if (this.navigator.MeshItemCount() > 1) {
        ...
        items.push({
          name: isMeshIsolated
            ? findName('RemoveIsolation')
            : findName('IsolateMesh'),
  ...
  FitMeshesToWindow(meshInstanceIdSet) {
    ...
    let boundingSphere = this.viewer.GetBoundingSphere((meshUserData) => {
      return meshInstanceIdKeys.has(
        meshUserData.originalMeshInstance.id.GetKey()
      );
    });
    this.viewer.FitSphereToWindow(boundingSphere, true);
  }
  ...
  UpdateMeshesSelection() {
    ...
    this.viewer.SetMeshesHighlight(this.highlightColor, (meshUserData) => {
      if (
        selectedMeshId !== null &&
        meshUserData.originalMeshInstance.id.IsEqual(selectedMeshId)
      ) {
        return true;
      }
      return false;
  ...
  InitToolbar() {
    ...
    let importer = this.modelLoaderUI.GetImporter();
    let navigationModeIndex =
      this.cameraSettings.navigationMode === NavigationMode.FixedUpVector
        ? 0
        : 1;
    let projectionModeIndex =
      this.cameraSettings.projectionMode === ProjectionMode.Perspective ? 0 : 1;

    AddButton(this.toolbar, 'open', findName('OpenFromYourDevice'), [], () => {
      this.OpenFileBrowserDialog();
    });
    AddButton(this.toolbar, 'open_url', findName('OpenFromUrl'), [], () => {
    ...
    AddRadioButton(
      ...
      [findName('FixedUpVector'), findName('FreeOrbit')],
      navigationModeIndex,
    ...
    AddRadioButton(
      ...
      [findName('PerspectiveCamera'), findName('OrthographicCamera')],
      projectionModeIndex,
      ['only_full_width', 'only_on_model'],
      (buttonIndex) => {
        if (buttonIndex === 0) {
    ...
    AddRadioButton(
      ...
      [findName('DarkMode'), findName('LightMode')],
      selectedTheme,
      ['align_right'],
      (buttonIndex) => {
        if (buttonIndex === 0) {
